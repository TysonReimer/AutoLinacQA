...
def _idx_to_mm(ds, do_x, image, edge_pixel_position, beam_CAX):
    ...
    if do_x:
        ...
        physical_edge = (edge_pixel_position - center_pixel[0]) * pixel_spacing[1]
        if beam_CAX is None:
            physical_edge += origin[0]

    else:  # If doing y
        # For a y-edge, convert the y coordinate relative to center:
        physical_edge = (edge_pixel_position - center_pixel[1]) * pixel_spacing[0]
        if beam_CAX is None:
    ...
    if indices.size != 0:  # If some indices were found
        ...
        if (idx > 5) and (idx < len(slice) - 5):
            if from_negative:
                ...
                frac = (threshold - slice[idx - 1]) / (slice[idx] - slice[idx - 1])
                edge_pos = (idx - 1) + frac
...
def _find_edge(image, ds, img_max, along_x=True, from_negative=True, beam_CAX=None):
...
def _get_dcm_jaw_pos(ds):
    ...
    x_jaws = ds.ExposureSequence[0].BeamLimitingDeviceSequence[0].LeafJawPositions
    y_jaws = ds.ExposureSequence[0].BeamLimitingDeviceSequence[1].LeafJawPositions
...
def _make_qat_dict_entry(jaw_dcm_pos, jaw_calc_pos, qat_dict, jaw_str):
    ...
    if this_str in qat_dict.keys():
        ...
        if isinstance(qat_dict[this_str], list):
            qat_dict[this_str] = qat_dict[this_str] + [jaw_calc_pos * cor_fac]
...
for ff in sorted(os.listdir(__D_DIR))[2:]:
    ...
    (
        ...
        y2_edge,
        y2_edge_mm,
    ) = _find_all_edges(image=image, ds=ds, beam_CAX=this_CAX, img_max=img_max)
    ...
    jaw_pos_list.append(
        ((x1, x2, y1, y2), (x1_edge_mm, x2_edge_mm, y1_edge_mm, y2_edge_mm))
    )
    ...
    plt.axvline(
        x=x1_edge, color="y", linestyle="-", linewidth=2, label="Detected X1-Edge"
    )
    plt.text(
        x1_edge + 50,
    ...
    # Draw a vertical red line at the detected x-edge position.
    plt.axvline(
        x=x2_edge, color="y", linestyle="--", linewidth=2, label="Detected X2-Edge"
    )
    plt.text(
        x2_edge + 50,
        float(center_pixel[0]),
    ...
    plt.axhline(
        y=y1_edge, color="red", linestyle="-", linewidth=2, label="Detected Y1-Edge"
    )
    plt.text(
    ...
    # Draw a horizontal red line at the detected y-edge position.
    plt.axhline(
        y=y2_edge, color="red", linestyle="--", linewidth=2, label="Detected Y2-Edge"
    )
    plt.text(
        float(center_pixel[1]),
        y2_edge + 50,
...
for ii in range(len(jaw_pos_list)):
    ((x1, x2, y1, y2), (x1_edge_mm, x2_edge_mm, y1_edge_mm, y2_edge_mm)) = jaw_pos_list[
        ii
    ]
    ...
    ij_images = _make_qat_dict_entry(
        jaw_dcm_pos=x1, jaw_calc_pos=x1_edge_mm, qat_dict=ij_images, jaw_str="X1"
    )
    ij_images = _make_qat_dict_entry(
        jaw_dcm_pos=x2, jaw_calc_pos=x2_edge_mm, qat_dict=ij_images, jaw_str="X2"
    )
    ij_images = _make_qat_dict_entry(
        jaw_dcm_pos=y1, jaw_calc_pos=y1_edge_mm, qat_dict=ij_images, jaw_str="Y1"
    )
    ij_images = _make_qat_dict_entry(
        jaw_dcm_pos=y2, jaw_calc_pos=y2_edge_mm, qat_dict=ij_images, jaw_str="Y2"
    )

for ii in list(ij_images.keys()):
    if "20" in ii:
        ij_images[ii] = (
            ij_images[ii][np.argmax(np.abs(np.array(ij_images[ii]) - 20))] / 10
        )
    elif "-20" in ii:
        ij_images[ii] = (
            ij_images[ii][np.argmax(np.abs(np.array(ij_images[ii]) + 20))] / 10
        ...
        ij_images[ii] = ij_images[ii][0] / 10
...
plt.figtext(0.5, 0.85, extra_text, wrap=True, horizontalalignment="center", fontsize=12)

# Create the table inside the figure.
table = plt.table(
    cellText=summary_data, colLabels=col_labels, loc="center", cellLoc="center"
)
