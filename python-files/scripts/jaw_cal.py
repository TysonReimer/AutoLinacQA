...
# Path to folder containing the .zip file of the .dcm files
...
def _find_CAX(ds_1, ds_2):
    ...
    -------
    beam_CAX :
        Indices of the MV CAX in image space
    """
...
def _idx_to_mm(ds, do_x, image, edge_pixel_position, beam_CAX):
    ...
    -------
    physical_edge :
        The physical edge of the jaw, in [mm]
    """
...
def _make_qat_dict_entry(jaw_dcm_pos, jaw_calc_pos, qat_dict, jaw_str):
    ...
    this_str = "%s %d" % (jaw_str, round(jaw_dcm_pos / 10, 0))
    ...
    if this_str in qat_dict.keys():
        ...
        if isinstance(qat_dict[this_str], list):
            ...
            qat_dict[this_str] = (
                    qat_dict[this_str] + [jaw_calc_pos - jaw_dcm_pos]
            )
...
if __name__ == "__main__":
    ...
    # Path for creating the pdf report
    pdf_report_path = os.path.join(
        get_proj_path(),
        "output/mech-qa/test-report.pdf"
    )

    # Path of the .zip file of interest
    zip_path = os.path.join(
        __D_DIR,
        "RTUK_Jaw_01Apr2025-001.zip"
    )

    tmp_dir = tempfile.mkdtemp()  # Make temporary dir

    # Load the zip file
    with zipfile.ZipFile(zip_path, 'r') as zip_ref:
        zip_ref.extractall(tmp_dir)

    __D_DIR = tmp_dir  # Set the data dir
    ...
    # Make summary_data for plt.table()
    summary_data = []
    qat_dict = dict()  # Make dict for QA Track integration
    ...
    for ii in list(qat_dict.keys()):
        qat_dict[ii] = np.max(qat_dict[ii])

    # Define column headers for the summary table.
    col_labels = ["X1 Diff (mm)", "X2 Diff (mm)", "Y1 Diff (mm)",
                  "Y2 Diff (mm)"]
    ...
    max_diff_x1 = np.max([abs(float(row[0])) for row in summary_data])
    max_diff_x2 = np.max([abs(float(row[1])) for row in summary_data])
    max_diff_y1 = np.max([abs(float(row[2])) for row in summary_data])
    max_diff_y2 = np.max([abs(float(row[3])) for row in summary_data])
