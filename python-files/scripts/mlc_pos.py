...
# __D_DIR = os.path.join(get_proj_path(), "data/mech-qa/mlc-pos/")
__D_DIR = os.path.join(
    "data/mech-qa/mlc-pos/",
    "RTUK MLC 22Apr2025 v2/"
)
...
def _make_qat_dict_entry(mlc_A_calc_pos, mlc_B_calc_pos,
    ...
    # Indices of leaves with max error in banks A/B
    ...
    qat_dict[this_str] = A_max_err, B_max_err  # Store result
...
def _find_leaf_indices_from_comb(comb_img, comb_ds):
    ...
    comb_ds :
        Loaded .dcm of the MLC-defined comb image
...
if __name__ == "__main__":
    ...
    pdf_report_path = os.path.join(
        get_proj_path(),
        "output/mech-qa/mlc-test-report.pdf"
    )
    ...
    if '2301' in comb_ds1.StationName:  # If RTUS
        ...
        # Hard-coded expected MLC positions
        MLC_pos = (
            np.vstack(
    ...
    # Init lists for storing info for pdf report
    img_fig_list = []
    all_errors = []
    ...
    image_number = 1  # Init image counter

    # Hard-code tolerances for pdf report generation; exceeding
    # tolerances results in colored text in pdf report
    tol_level = 2  # Level 1 tolerance, 2 mm
    action_level = 5  # Level 2 tolerance, 5 mm

    mlc_pos_list = []  # Init list for QA Track reporting
    ...
    for ff in sorted(os.listdir(__D_DIR))[4:]:
        ...
        image = ds.pixel_array  # Get image arr
        ...
        # Store calculated/measured and planned positions
        mlc_pos_list.append(
        ...
        # Create figure for this image (1 page in pdf report)
        ...
        ax_Bbar = fig.add_subplot(gs[0, 1])  # top-right
        ax_Abar = fig.add_subplot(gs[1, 1])  # bottom-right

        # title and worst‐leaf text
        fig.suptitle(f"Image {image_number}", fontsize=16, y=0.96)
        fig.text(0.5, 0.88,
                 f"Bank A: Worst leaf was {wA_idx + 1} with "
                 f"position error {wA_err:.2f} mm\n"
                 f"Bank B: Worst leaf was {wB_idx + 1} with "
                 f"position error {wB_err:.2f} mm",
                 ha="center")

        # image and overlay lines and plan vs meas text
        ...
        ax_img.text(cx, A_edge + 150,
                    f"Meas: {A_edge_mm:.2f} mm\nPlan: {bank_A_pos:.2f} mm"
                    f"\nMedian Diff: {A_edge_mm - bank_A_pos:+.2f} mm",
                    fontsize=9, color="red",
                    bbox=dict(facecolor="white", alpha=0.9, edgecolor="black",
                              boxstyle="round"))
        ax_img.text(cx, B_edge + 150,
                    f"Meas: {B_edge_mm:.2f} mm\nPlan: {bank_B_pos:.2f} mm"
                    f"\nMedian Diff: {B_edge_mm - bank_B_pos:+.2f} mm",
                    fontsize=9, color="red",
                    bbox=dict(facecolor="white", alpha=0.9, edgecolor="black",
                              boxstyle="round"))
        ...
        # Bank B bar chart (highlight worst in red)
    ...
    # list any alerts on the summary page
    ...
    qat_dict = dict()  # Init dict for QA Track integration

    # For each target image
    for ii in range(len(mlc_pos_list)):
